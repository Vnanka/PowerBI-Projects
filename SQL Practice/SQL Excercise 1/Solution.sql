...
FROM 
		(SELECT a.* from
			...
			,	(SELECT emp_no	FROM dept_manager 
					WHERE	emp_no = 110022) Manager_ID
			FROM employees e
		...
		SELECT b.* from
			...
			,	d.dept_name Deparmtent_Name 		-- Code Enhancement on top of the main task
			,	(SELECT emp_no	FROM dept_manager 
					WHERE	emp_no = 110039) Manager_ID
			FROM employees e
			JOIN dept_emp de ON e.emp_no = de.emp_no 
			JOIN departments d on d.dept_no = de.dept_no 	-- Code Enhancement on top of the main task
    ...
    -- Despite supposed 42 rows the query returns 44 with 2 additionals rows with dept_no duplicates.
